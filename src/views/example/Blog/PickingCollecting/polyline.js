...
import { graphicFill, graphicWidth, modifyAlpha, pointPixelSize } from './index.js';
...
export const entityCreate = () => {
    ...
    if (editor.adding) {
        ...
        screenEvent.setInputAction((__event__) => {
            ...
            if (addPoints.length > 1 && !addPolyline) {
                addPolyline = viewer.entities.add({
                    name: `${ label }-${ entities.length + 1 }`,
                    polyline: {
                        positions: new Cesium.CallbackProperty(() => addPoints, false),
                        width: pointPixelSize / 3,
            ...
            if (!eventKeyup) {
                ...
                let cancel = () => {
                    ...
                    addPointIds = [];
                    addPoints = [];
                    viewer.entities.remove(addPolyline);
                    addPolyline = null;
                    editor.wating = true;
                    editor.creating = false;
                    eventKeyup = null;
        ...
        screenEvent.setInputAction(async () => {
            ...
            if (Cesium.defined(addPolyline)) {

                addPolyline.polyline.positions = addPoints.map(d => {
                    return new Cesium.Cartesian3(d.x, d.y, d.z);
                });

                entities.push({ id: addPolyline.id, name: addPolyline.name, graphicFill, graphicWidth, category });
...
export const entityModify = async (item) => {
    ...
    editPolyline = viewer.entities.getById(item.id);

    if (editor.adding || editor.modifying || editor.flying || !Cesium.defined(editPolyline)) {
        return;
    }
    ...
    await entityAim(editPolyline);

    let { __graphic__, name } = editPolyline;
    let graphic = editPolyline[ __graphic__ ];
    graphicBackup = graphic.clone();

    editor.category = __graphic__;
    editor.id = editPolyline.id;
    editor.name = name;
    editor.graphicFill = graphic.material.color.getValue(time).toCssColorString();
    editor.graphicWidth = graphic.width.getValue(time);
    let { degrees } = entityDegrees(editPolyline);
    editor.coordinates = degrees;
    editor.description = editPolyline.description;
    editor.modifying = true;
    editorBackup = cloneDeep(editor);

    entityCode();

    let cartesianCollection = editPolyline.polyline.positions.getValue(time);
    ...
    for (let i = 0; i < cartesianLength; i++) {
        ...
        let point = addPoint(viewer, current, 'polyline-edit-point', {
            color: Cesium.Color.ORANGE.toCssColorString()
        });
        ...
        if (i < cartesianLength - 1) {
            ...
            let pointHalf = addPoint(viewer, cartesianHalf, 'polyline-edit-point-midpoint');
    ...
    editPolyline.polyline.positions = new Cesium.CallbackProperty(() => editNodes, false);
    ...
    screenEvent.setInputAction((__event__) => {
        ...
        if (!entityMoveEnable && !entityMovingPoint) {
            ...
            if (pickEntity.name === 'polyline-edit-point') {

                entityMovingPoint = pickEntity;
                entityMoveEnable = true;
                editNodeIdIndex = editNodeIds.findIndex(d => d === pickEntity.id);

                if (editHalfNodeIds[ editNodeIdIndex - 1 ]) {
                    if (!editHalfPrevEntity || editHalfPrevEntity.id !== editHalfNodeIds[ editNodeIdIndex - 1 ]) {
                        editHalfPrevEntity = viewer.entities.getById(editHalfNodeIds[ editNodeIdIndex - 1 ]);
                    }
                }

                if (editHalfNodeIds[ editNodeIdIndex ]) {
                    if (!editHalfNextEntity || editHalfNextEntity.id !== editHalfNodeIds[ editNodeIdIndex ]) {
                        editHalfNextEntity = viewer.entities.getById(editHalfNodeIds[ editNodeIdIndex ]);
                    }
                }
    ...
    screenEvent.setInputAction((__event__) => {
        ...
        if (entityMovingPoint.name === 'polyline-edit-point') {
            ...
            if (Cesium.defined(editHalfPrevEntity)) {
                ...
                let currPrev = editNodes[ editNodeIdIndex - 1 ];
            ...
            if (Cesium.defined(editHalfNextEntity)) {
                ...
                let currNext = editNodes[ editNodeIdIndex + 1 ];
    ...
    screenEvent.setInputAction(() => {
        ...
        if (pickEntity && pickEntity.name === 'polyline-edit-point') {
    ...
    screenEvent.setInputAction((__event__) => {
        ...
        if (pickEntity.name === 'polyline-edit-point') {
            ...
            let prevPointId = editNodeIds[ prevIndex ];

            let nextIndex = editNodeIdIndex + 1;
            if (nextIndex === editNodeIds.length) {
                nextIndex = -1;
            }
            let nextPointId = editNodeIds[ nextIndex ];
            ...
            let halfPointId = editHalfNodeIds[ halfIndex ];
            viewer.entities.removeById(halfPointId);
            editHalfNodeIds.splice(halfIndex, 1);

            let prevPointIndex = editNodeIds.findIndex(d => d === prevPointId);
            let nextPointIndex = editNodeIds.findIndex(d => d === nextPointId);

            let setHalfPointIndex = halfIndex - 1;
            if (setHalfPointIndex < 0) {
                return;
            }
            let setHalfPointId = editHalfNodeIds[ setHalfPointIndex ];

            if (nextPointIndex === -1) {
                viewer.entities.removeById(setHalfPointId);
                return;
            }

            let ppi = editNodes[ prevPointIndex ];
            let pni = editNodes[ nextPointIndex ];

            if (ppi && pni) {

                let editHalfEntity = viewer.entities.getById(setHalfPointId);
        ...
        if (pickEntity.name === 'polyline-edit-point-midpoint') {

            pickEntity.name = 'polyline-edit-point';
            ...
            if (editNodeIds[ prevIndex ]) {
                ...
                editHalfPrevEntity = addPoint(viewer, ph, 'polyline-edit-point-midpoint');
            ...
            if (editNodeIds[ nextIndex ]) {
                ...
                editHalfNextEntity = addPoint(viewer, nh, 'polyline-edit-point-midpoint');
...
export const entityModifyPosition = (index) => {
    ...
    let editPointId = editNodeIds[ index ];
    ...
    entityMovingPoint = viewer.entities.getById(editPointId);

    if (Cesium.defined(entityMovingPoint)) {
        entityMovingPoint.position = cartesian;
    }
...
export const entityModifyConfirm = async () => {
    ...
    if (entity) {
        ...
        entity.graphicWidth = editor[ 'graphicWidth' ];
...
export const entityModifyRestore = () => new Promise((resolve, reject) => {
    ...
    try {
        ...
        graphic.width = backupGraphic.width;
        ...
        graphic.positions = positionsBackup.map((d, i) => {
...
export const entityModifyReset = () => {
    ...
    editPolyline.polyline.positions = editNodes.map(d => {
        return new Cesium.Cartesian3(d.x, d.y, d.z);
    });
...
export const entityCopyApply = (item) => {
    ...
    if (Cesium.defined(sourceGraphic) && Cesium.defined(targetGraphic)) {
        ...
        targetGraphic.width = sourceGraphic.width.getValue(time);
...
export const entityDegrees = (entity) => {
    ...
    let cartesian = entity.polyline.positions.getValue(time);
